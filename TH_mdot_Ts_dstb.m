...
for i=1:steps
    ...
    % Controllers
    Ts=300-20*(T-Td)-0.5*intT;
    mdot=0.2-500*(w-wd)-2*intw;
%     mdot = 0.2;
...
subplot(2,2,4)
pbaspect([2,1,1]);
plot(time, Ts_traj,'LineWidth',1.5);hold on;
plot(time, repmat(TsH,1,size(time,2)),'r','LineWidth',1.5);
xlabel('Time(s)','FontSize',fontsize);yl=ylabel('$T_s$ (K)','FontSize',fontsize);
lg=legend('$T_s$', '$T_{sH}$','location','southeast');
set(gca,'fontsize',fontsize)
set(lg, 'Interpreter', 'latex');
set(yl, 'Interpreter', 'latex');
axis([0 200 294 302])
...
% % plot(time(1:i),Tinf_traj(1:i),'Color','r','LineWidth',1.5);
...
% % subplot(3,2,6);
% % pbaspect([2,1,1])
% % plot(time(1:i), Ts_traj(1:i),'LineWidth',1.5);hold on;
% % set(gca,'fontsize',15)
% % plot(time, repmat(TsH,1,size(time,2)),'r');
% % xlabel('Time(s)','FontSize',fontsize);ylabel('Ts','FontSize',fontsize);
% % 
% % 
% % 
% % 
